...
def create_pipeline_ensemble(plant_test, pipeline_preprocessing: Pipeline,
    ...
    if adaption_configuration == AdaptionConfiguration.none:
        ...
        weight_optimization = "autoOpt"  # least squares ensemble optimization

    ensemble_model_pool = {}
    for name in modules.keys():
        if "scaler" in name or "features" in name:
            continue
    ...
    if adaption_configuration == adaption_configuration.AFB:
        refit_conditions = [PeriodicCondition(name="PeriodicEnsemble",
                                              ...
                                              refit_batch=K,
                                              refit_params={"weights": weight_optimization})]
    ...
    ensemble = Ensemble(weights=weight_optimization, name=f"ensemble_{plant_test}")(
